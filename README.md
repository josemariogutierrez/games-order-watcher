...
## Telling the two watchers apart

Every alert is stamped with the machine that found the post:

| | Title | Footer |
|---|---|---|
| Mac | `💻 Game drop: zelda` | `Detectado por: Mac (local)` |
| GitHub | `☁️ Game drop: zelda` | `Detectado por: GitHub (respaldo)` |

Detection is automatic - GitHub Actions always sets `GITHUB_ACTIONS=true`, and
anything else is treated as the Mac. Set `WATCHER_SOURCE` to override it if this
ever runs somewhere else.

A run of `☁️` alerts means the Mac is not doing its job: asleep, or the agent
stopped. Worth checking `./status.sh` when you notice it.
